...
# ks = [1, 50, 100]
def bench_map_single(ns, ks, number=10_000, seed=42):
    ...
    def our_callback(n, k, init_out):
        preds, target = init_out
        return mean_average_precision(preds, target, k=None, reduction='mean')
    ...
    g.fig.suptitle('Benchmark - Single Example: Mean Average Precision', fontsize=16)
    return df, g

# n=512,
# k=50,
# batch_sizes=[32, 64, 128, 256, 512],
def bench_map_batch(n, k, batch_sizes, number=1_000, seed=42):
    ...
    def base_callback(n, k, init_out):
        preds, target = init_out
        out = torch.stack([
            retrieval_average_precision(_preds, _target)
    ...
    g.fig.subplots_adjust(top=.88)
    g.fig.suptitle('Benchmark - Mini-Batch: Mean Average Precision', fontsize=16)
    return df, g
