...
# ks = [1, 50, 100]
def bench_ndcg_single(ns, ks, number=10_000, seed=42):
    ...
    def our_callback(n, k, init_out):
        preds, target = init_out
        return normalized_dcg(preds, target, k=k, reduction='mean')
    ...
    g.fig.suptitle('Benchmark - Single Example: NDCG', fontsize=16)
    return df, g

# n=512,
# k=50,
# batch_sizes=[32, 64, 128, 256, 512],
def bench_ndcg_batch(n, k, batch_sizes, number=1_000, seed=42):
    ...
    def base_callback(n, k, init_out):
        preds, target = init_out
        out = torch.stack([
            retrieval_normalized_dcg(_preds, _target, k=k)
    ...
    g.fig.subplots_adjust(top=.88)
    g.fig.suptitle('Benchmark - Mini-Batch: NDCG', fontsize=16)
    return df, g
